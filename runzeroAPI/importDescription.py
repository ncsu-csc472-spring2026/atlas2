...
if len(sys.argv) < 4:
    print("Usage: python3 add_asset_scan.py <SITE_NAME> <IP1> [<IP2> ...] <DESCRIPTION>")
    sys.exit(1)

SITE_NAME_INPUT = sys.argv[1]

# Last argument is description, everything between is IPs
...
try:
    ...
    for cidr in CIDRS:
        if cidr not in existing_subnets:
            ...
            print(f"[+] Subnet {cidr} added with description.")
            updated = True
        else:

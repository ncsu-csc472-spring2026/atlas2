...
# Config, grab .env file and get API token
BASE_URL = "https://console.runzero.com/api/v1.0/org"
# Load .env file from package lib/ directory always
load_dotenv(files("atlas2").joinpath(".env"))
RUNZERO_API_TOKEN = os.getenv("RUNZERO_API_TOKEN")
...
def create_site(site_name):
    ...
    r = requests.put(f"{BASE_URL}/sites", headers=HEADERS, json=payload, timeout=30)
    r.raise_for_status()
    print(f"[+] Created runZero site '{site_name}'")
    return r.json()
...
def update_site_subnets(site_id, subnets, site_name):
    ...
    r.raise_for_status()
    print(f"[+] Updated runZero site {site_name}")
    return r.json()
...
def trigger_scan(site_id, cidrs):
    ...
    print(f"[+] Initialted runZero scan for the updated site")
